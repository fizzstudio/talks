  ...
  if (window.CustomEvent) {
    ...
    for (var i = 0; i < this.bullets.length; ++i) {
      if (1 == this.b && "next" == dir) {
        const bulletElement = this.bullets[i];

        if (!bulletElement.classList.contains('scroll')) {
...
silver_bullet.prototype.show = function (e) {
  ...
  const bulletElement = e.currentTarget;

  if (bulletElement.classList.contains('scroll')) {
    console.log('scroll', bulletElement);
    bulletElement.scrollIntoView({ behavior: "smooth", block: "start", inline: "nearest" });
  } else {
    if (this.b > 23) {
    ...
    if (bulletElement.beginElement) {
      bulletElement.beginElement();
    }
  }
...
silver_bullet.prototype.hide = function (e) {
  ...
  if (bulletElement.classList.contains('scroll')) {
    console.log('scroll', bulletElement);
    bulletElement.scrollIntoView({ behavior: "smooth", block: "end", inline: "nearest" });
  } else {
    // console.log("hide");
    bulletElement.style.visibility = "hidden";
...
silver_bullet.prototype.draw = function (e) {
  // console.log("draw");
  var bulletElement = e.currentTarget;
  let draw = bulletElement.getAttribute("data-draw");
  ...
  bulletElement.style.visibility = "visible";
  bulletElement.classList.add(draw_class);
}
